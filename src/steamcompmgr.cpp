...
// 				fprintf( stderr, "grab?\n" );
	...
	m_texture = vulkan_create_texture_from_bits(m_surfaceWidth, m_surfaceHeight, VK_FORMAT_B8G8R8A8_SRGB, texCreateFlags, cursorBuffer.data());
	assert(m_texture);
	XFree(image);
	m_dirty = false;
		...
		{
			{
				...
				for (size_t i = 0; (server = wlserver_get_xwayland_server(i)); i++)
				{
					for (win *w = server->ctx->list; w; w = w->next)
					{
						...
						if ( w == global_focus.focusWindow && !fpslimit_use_frame_callbacks_for_focus_window() )
							bSendCallback = false;
