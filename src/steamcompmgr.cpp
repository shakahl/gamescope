		...
		{
			{
				...
				{
					if ( !g_HeldCommits[HELD_COMMIT_BASE].done || g_HeldCommits[HELD_COMMIT_FADE].commitID != g_HeldCommits[HELD_COMMIT_BASE].commitID )
...
found:
	...
	{
		if ( FADE_OUT_DURATION != 0 && prevFocusWindow != None )
		{
			if ( !g_HeldCommits[ HELD_COMMIT_FADE ].done )
			{
			...
			else
			{
				...
				if ( g_HeldCommits[ HELD_COMMIT_FADE ].fb_id == g_HeldCommits[ HELD_COMMIT_BASE ].fb_id ||
					g_HeldCommits[ HELD_COMMIT_FADE ].vulkanTex == g_HeldCommits[ HELD_COMMIT_BASE ].vulkanTex )
				{
					g_HeldCommits[ HELD_COMMIT_FADE ] = commit_t{};
					g_bPendingFade = false;
				}
		...
		for ( win *w = list; w; w = w->next )
		{
			...
			for ( j = 0; j < w->commit_queue.size(); j++ )
			{
				if ( w->commit_queue[ j ].commitID == listCommitsDone[ i ] )
				{
					...
					{
						w->commit_queue[ j ].held = true;
						if ( g_HeldCommits[ HELD_COMMIT_BASE ].done )
						{
							if ( !g_HeldCommits[HELD_COMMIT_FADE].done || g_HeldCommits[HELD_COMMIT_FADE].commitID != g_HeldCommits[HELD_COMMIT_BASE].commitID )
