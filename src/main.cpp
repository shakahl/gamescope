...
int main(int argc, char **argv)
{
	...
	if ( lease_fd != -1 )
	{
		printf( "Got a lease fd!\n" );
		g_bIsLeased = true;
	}

	if ( lease_fd == -1 && ( getenv("DISPLAY") != NULL || getenv("WAYLAND_DISPLAY") != NULL ) )
	{
		g_bIsNested = true;
	}
...
static bool initOutput( int preferredWidth, int preferredHeight, int preferredRefresh, int lease_fd )
{
	...
	{
		return init_drm( &g_DRM, preferredWidth, preferredHeight, preferredRefresh, lease_fd );
	}
}
